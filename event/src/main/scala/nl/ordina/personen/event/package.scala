...
import nl.ordina.personen.event.cassandra.{CassandraClient, CassandraStorageEngine, CassandraTransactionManager}
import org.axonframework.eventsourcing.eventstore.EmbeddedEventStore

package object event {

  private lazy val cluster = Cluster.builder().addContactPoint("127.0.0.1").build()
  lazy val session = cluster.connect("brp")
  private lazy val client = new CassandraClient(session)
  lazy val transactionManager = new CassandraTransactionManager(client)
  private lazy val eventStorageEngine = new CassandraStorageEngine(transactionManager, client)
